...
| External-check waiver comment (`idd-external-check-waiver`) | Human maintainer only, under the repository's `ciGate.externalCheckWaivers` policy — plus one narrow, documented exception: `github-actions[bot]`, for the `idd-advisory-convergence` selector's own `self-referential-bootstrap-auto` reason token only, when a PR's diff touches that check's own committed trigger-file allowlist (kurone-kito/idd-skill#2657); this repository's `idd-advisory-convergence.yml` hosts that self-waiver job (reconciled to the v0.11.0 pin in #163), so this bot-posted exception is live here for PRs touching that allowlist | Not mutated once posted; superseded by a fresh waiver | F2/F3 CI gate (`waiverEvidence` checks); the bot-posted kind is additionally verified by resolving its `run-id:` field to a `pull_request_target`-triggered run of `idd-advisory-convergence.yml` for the current PR HEAD SHA |
