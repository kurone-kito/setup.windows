...
このうち `PendingFileRenameOperations` は既知の誤検知源である(一部の
アンチウイルス製品がこのキーに残骸を残し、通常の再起動では解消しない
ことがある)。

`setup.cmd` は現在、Boxstarter を呼び出す直前(Chocolatey / Boxstarter の
導入ステップの後)に事前チェック(`libs/reboot-guard.ps1` の
`Test-PendingRebootIndicators`)を実行する。これらの導入ステップ自体が
再起動保留状態を残した場合でも検知できる位置になっている。いずれかの
指標が立っていれば、どの指標かを表示したうえで中断し、上記のような
無限ループに陥ることを防ぐ。
...
# コンピューター名変更保留(不一致、またはいずれかのキーの存在):
